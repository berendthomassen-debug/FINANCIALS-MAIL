...
def ai_sections(markets, crypto, fx, commodities, news, date_str, fng=None, recession=None):
    ...
    if not isinstance(result, dict):
        print("  → onverwachte structuur, vangnet gebruikt", file=sys.stderr)
        return fallback

    LIJSTVELDEN = ("kernpunten", "opmerkelijk", "analyse", "macro_agenda",
                   "scenarios", "verwachtingen", "risico_radar", "per_coin")
    TEKSTVELDEN = ("samenvatting", "hoofdverhaal", "macro_uitleg",
                   "portfolio_reflectie", "speculatie")

    for key in LIJSTVELDEN:
        w = result.get(key)
        if isinstance(w, dict):
            w = [w]                       # één object i.p.v. een lijst
        elif isinstance(w, str):
            w = [w] if w.strip() else []  # losse tekst i.p.v. een lijst
        if not isinstance(w, list):
            w = []
        result[key] = [x for x in w if x not in (None, "", {}, [])]

    for key in TEKSTVELDEN:
        w = result.get(key)
        if isinstance(w, (list, tuple)):
            w = "\n".join(str(x) for x in w)
        result[key] = str(w) if w else ""
...
def veld(item, sleutel, standaard=""):
    """Veilig een veld uit AI-uitvoer halen: ontbrekende sleutels, verkeerde
    types en losse teksten leiden nooit tot een crash."""
    if isinstance(item, dict):
        w = item.get(sleutel, standaard)
    ...
    if isinstance(item, str):
        return item if sleutel in ("tekst", "verwachting", "toelichting",
                                   "gebeurtenis", "titel", "onderwerp") else standaard
    return standaard
...
def render(cfg, markets, crypto, fx, commodities, news, ai, date_str, fng=None, recession=None):
    ...
    coin_teksten = {veld(c, "ticker"): veld(c, "tekst")
                    for c in ai.get("per_coin", []) if isinstance(c, dict)}
    ...
    kernpunten_html = "".join(
        f'<li>{refs_to_links(html.escape(str(k)), news)}</li>'
        for k in ai.get("kernpunten", []) if str(k).strip())

    opmerkelijk = "".join(
        f'<div class="kaart"><div class="kaart-kop"><h4>{html.escape(veld(o, "titel", "Opmerkelijk"))}</h4></div>'
        f'<p>{refs_to_links(html.escape(veld(o, "tekst")), news)}</p></div>'
    ...
    for a in ai.get("analyse", []):
        analyse_map[veld(a, "categorie").lower()] = paras(veld(a, "tekst"), news)
    ...
    agenda = "".join(
        f'<tr><td class="ag-datum">{html.escape(veld(a, "datum", "—"))}</td>'
        f'<td>{html.escape(veld(a, "gebeurtenis"))}<span class="sub">{html.escape(veld(a, "impact"))}</span></td>'
        f'<td><span class="badge {badge.get(veld(a, "belang", "middel").lower(), "b-laag")}">{html.escape(veld(a, "belang", "middel"))}</span></td></tr>'
        for a in ai.get("macro_agenda", []))

    scenarios = "".join(
        f'<div class="kaart scen-{veld(s, "naam", "basis").lower()}"><div class="kaart-kop"><h4>{html.escape(veld(s, "naam", "Scenario"))}</h4>'
        + (f'<span class="badge b-middel">kans: {html.escape(veld(s, "kans"))}</span>' if veld(s, "kans") else "")
        + f'</div><p>{refs_to_links(html.escape(veld(s, "tekst")), news)}</p></div>'
        for s in ai.get("scenarios", []))

    verwachtingen = "".join(
        f'<div class="kaart"><div class="kaart-kop"><h4>{html.escape(veld(v, "onderwerp", "Verwachting"))}</h4>'
        + (f'<span class="badge b-laag">{html.escape(veld(v, "horizon"))}</span>' if veld(v, "horizon") else "")
        + f'</div><p>{refs_to_links(html.escape(veld(v, "verwachting")), news)}</p>'
        + (f'<div class="kaart-voet"><span class="badge {badge.get(veld(v, "vertrouwen").lower(), "b-laag")}">vertrouwen: {html.escape(veld(v, "vertrouwen"))}</span></div>' if veld(v, "vertrouwen") else "")
        + '</div>'
        for v in ai.get("verwachtingen", []))

    risico = "".join(
        f'<tr><td class="naam">{html.escape(veld(r, "risico", "Risico"))}</td>'
        f'<td><span class="badge {badge.get(veld(r, "kans", "middel").lower(), "b-laag")}">kans: {html.escape(veld(r, "kans", "?"))}</span></td>'
        f'<td><span class="badge {badge.get(veld(r, "impact", "middel").lower(), "b-laag")}">impact: {html.escape(veld(r, "impact", "?"))}</span></td>'
        f'<td class="dim">{html.escape(veld(r, "toelichting"))}</td></tr>'
...
def main():
    ...
    try:
        ...
        print(f"  ! renderen mislukte ({e}) — brief wordt regelgebaseerd opgebouwd",
              file=sys.stderr)
        ai = rule_based_sections(markets, crypto, fx, commodities, news, date_str,
                                 fng=fng, recession=recession)
        render(cfg, markets, crypto, fx, commodities, news, ai, date_str,
               fng=fng, recession=recession)
